...
<!-- 意向数据表格 开始 -->
	<table class="layui-hide" id="intentionTable" lay-filter="intentionTable"></table>
	<!--头工具栏 -->
	<div style="display: none;" id="intentionToolBar">
       <!-- <button type="button" class="layui-btn layui-btn-danger layui-btn-sm" lay-event="deleteBatch">批量删除</button> -->
	</div>
	<!-- 行工具 -->
	<div  id="intentionBar" style="display: none;">
...
layui.use([ 'jquery', 'layer', 'form', 'table', 'laydate' ], function() {
	...
	tableIns=table.render({
	    elem: '#intentionTable' //渲染的目标对象
	    ,url:'${ctx}/intention/getAllIntention' //数据接口
	    ,toolbar: '#intentionToolBar' //开启头部工具栏，并为其绑定左侧模板
	    ...
	    ,cols: [[
	      ...
	      ,{field:'intentionId', title:'ID',align:'center',sort: true}
	      ...
	      ,{field:'intentionTime', title:'添加时间', align:'center',width:200,sort: true}
	      ,{fixed: 'right', title:'操作', toolbar: '#intentionBar',width:150, align:'center'}
	    ]],
	...
	function lopenView(data){
		mainIndex=layer.open({
			...
			success:function(index){
				$.post("${pageContext.request.contextPath}/intention/getIntentionById",{intentionId:data.intentionId},
						function(intention){
					console.log(intention);
					$("#view_img").attr("src","${ctx}/file/downloadShowFile?path="+intention.sample.sampleImage);
				})
			}
		});
	};
	...
	$("#doSearch").click(function(){
		...
		tableIns.reload({
			url:"${ctx}/intention/getAllIntention?"+params
			,page:{curr:1}		
		})
	});
	//监听头部工具栏事件
	table.on("toolbar(intentionTable)",function(obj){
		 switch(obj.event){
		    case 'deleteBatch':
		      deleteBatch();
			break;
	...
	//监听行工具事件
	   table.on('tool(intentionTable)', function(obj){
		   var data = obj.data; //获得当前行数据
		   var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
		  if(layEvent === 'del'){ //删除
			  layer.confirm('确定删除ID为“'+data.intentionId+'”的意向吗?', function(index){
			       //向服务端发送删除指令
			       $.post("${ctx}/intention/deleteIntentionByIntentionId",{intentionId:data.intentionId},function(res){
			    	   layer.msg(res.msg);
			    	    //刷新数据 表格
						tableIns.reload();
			       })
	...
	function deleteBatch(){
		//得到选中的数据行
		var checkStatus = table.checkStatus('intentionTable');
	    var data = checkStatus.data;
	    if(checkStatus.data.length>0){
	    layer.confirm('确定要删除选中的意向吗?', function(index){
		    	 //将对象数组中的 intentionId属性 转成数组
			    var result = data.map(function(item) {
		            return item.intentionId;
				});
		    	 //向服务端发送删除指令
		       $.ajax({
		    	   url:"${ctx}/intention/deleteIntentionByArray",
		    	   type:'post',
		    	   data:"ids="+result,
